...
apply_handle_commit(StringInfo s)
{
	...
	switch (apply_action)
	{
		case TRANS_LEADER_APPLY:
			/*
			 * XXX: Apart from parallelized transactions, we do not have to
...
apply_handle_commit_prepared(StringInfo s)
{
	...
	/* Delete the transaction from parallelized key */
	leader_finish_transaction(prepare_data.xid);

	/*
	 * XXX no need to update last_remote_xid because upcoming transaction
	 * preserves the order automatically. Let's set the xid to an invalid
	 * value to skip sending the INTERNAL_DEPENDENCY message.
	 */
...
apply_handle_rollback_prepared(StringInfo s)
{
	...
	/* Delete the transaction from parallelized key */
	leader_finish_transaction(rollback_data.xid);
