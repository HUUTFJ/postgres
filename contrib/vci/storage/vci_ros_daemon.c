...
vci_ROS_control_daemon_setup(void)
{
	...
	snprintf(worker.bgw_name, BGW_MAXLEN, "vci:ROS control daemon");
	snprintf(worker.bgw_type, BGW_MAXLEN, "vci:ROS control daemon");
...
vci_ROS_control_daemon_main(Datum main_arg)
{
	...
	/* I am autovacuum launcher */
	vci_AutovacuumLauncherIAm();

	BackgroundWorkerInitializeConnection(NULL, NULL, 0);	/* Connect to Shared
															 * database */
	vci_AutovacuumLauncherNotIAm();

	/* Connect DB to accerss common system catalog */
