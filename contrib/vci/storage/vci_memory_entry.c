 ...
 *
 * @param[in] vciid id of vci, information of which are stored in the found memory entry.
 * @return The index to the found object.
 *
 * @node This function must be called
 * under the lock of the vci_memory_entries_t is acquired exclusively.
 * Since this function does not acquire any lock on the entry,
 * user must lock the entry and check if it has the correct OID.
 */
static int
 ...
 *
 * @param[in] vciid id of vci whose information is stored in the determined memory entry.
 * @param[in] tsid tablespace oid
 *
 ...
 *
 * @param[out] entry    memory entry to be released
 */
static void
