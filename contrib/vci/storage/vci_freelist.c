 ...
 *
 * IDENTIFICATION
 *		contrib/vci/storage/vci_freeslist.c
 */
...
/**
 * fucntion to cast from Page to (vcis_freespace_t *)
 */
...
#if 0
	Assert(freeSpace->prev_pos != startBlockNumber);
	Assert(freeSpace->next_pos != startBlockNumber);
#endif
...
#if 0
	Assert((prevFreeBlockNumber != nextFreeBlockNumber) ||
		   (!BlockNumberIsValid(nextFreeBlockNumber)));
#endif
