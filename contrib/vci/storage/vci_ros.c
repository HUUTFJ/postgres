 ...
 * relation.
 * This function "just" initialize the give object.
...
CheckRosVersion(vci_MainRelHeaderInfo *info)
{
	...
	if ((major == 0) && (minor == 0))
		ereport(ERROR, (errmsg("ROS has not been formated yet."),
 ...
 * recovery, or vci_wmrv_all for all pages.
 * The latter should only be used in building the index.
 */
void
...
/**
 * @brief
 * This function checks if the relation has the DB page with the page ID
 * blockNumber.
 ...
 */
static void
InitPageCoreWithoutLock(Buffer buffer, int16 numItems)
{
	uint32		size;
	uint32		itemSize;
	int32		aId;
...
vci_InitPageCore(Buffer buffer, int16 numItems, bool locked)
{
	...
	InitPageCoreWithoutLock(buffer, numItems);
...
/**
 * @brief
 * This function get or newly create a DB buffer page, and put the
...
/**
 * @brief
 * This function mark the buffer dirty, and make WAL from the item
 * in the buffer.
...
/**
 * @brief
 * This function first compares blockNumber and blockNumberOld.
 ...
 * @return The bit ID in null bit vector.
 * For not nullable columns, it returns -1.
 */
int16
...
/**
 * @brief Get the column widths in the worst cases.
...
/* search for free extent and return the extent ID.
 * This function read extent information in the ROS main relation and check
...
/**
 * @brief
 * This function check if the extentID is 0 <= extentID and
 ...
 * It might be happen that the page is vanised in some trouble...?
...
ReadBufferWithPageInitCore(Relation reln, BlockNumber blockNumber, int16 numItem)
{
	...
	if (PageIsNew(page))
	{
		LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
		if (PageIsNew(page))
			InitPageCoreWithoutLock(buffer, numItem);
		LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
	}
