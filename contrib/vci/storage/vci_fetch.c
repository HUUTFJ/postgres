							 ...
							 bool useCompression)
{
	...
	Assert(useCompression == false); /* Compression code has been extracted from the contrib/vci module */
 ...
 * @details
 * Check if the extent is visible with the relation among current ROS version
 * and \c Xgen, \c Xdel.
 ...
 * @details
 * We have some ways to estimate the number of rows in data WOS.
 ...
 * @parameter[in] queryContext estimate the size of local ROS for the given
 * query context.
 ...
 *
 * @details
 * The function \c vci_CSFetchVirtualTuples() reads and stores multiple rows
 * at once from the specified columns and rows.
 ...
 * @parameter[in] fetchContext the fetch context.
 * @parameter[in] numRows required number of rows read at once.
...
/**
 * @breaf Destroy given instance of \c vci_virtual_tuples_t.
 * @parameter[in] vTuples target to be destory.
 */
void
...
/**
 * @breaf Fill CRIDs in the given \c vci_virtual_tuples_t.
 * @parameter[in] vTuples target virtual tuples.
 */
void
 ...
 *
 * @details
 * vci_CSFetchVirtualTuples returns number of rows which can be read from
 * stored data after cridStart.  For example, if cridStart = 50,
 * but actualNumberOfRowsReadAtOnce = 128,
 * vci_CSFetchVirtualTuples() returns 78 (= 128 - 50).
 *
 ...
 * @parameter[in,out] vTuples the read data are stored in the pointed area.
 * @parameter[in] cridStart the data from \c cridStart row are read.
 * @parameter[in] numReadRows required number of rows to be read.
 ...
 * The result is stored in a \c palloc()ed area.
 * Thus, caller should \c pfree() the result after use.
