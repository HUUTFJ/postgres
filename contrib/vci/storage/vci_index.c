...
static Bitmapset *vci_create_dropped_column_ids(TupleDesc tableTupdesc);
static bool is_empty_cstring(const char *target);
...
bool		vci_canreturn(Relation index, int attno);
...
bool		vci_gettuple(IndexScanDesc scan, ScanDirection dir);
bool		vci_validate(Oid opclassoid);
int64		vci_getbitmap(IndexScanDesc scan, TIDBitmap *tbm);
...
vci_isVciAdditionalRelationTuple(Oid reloid, Form_pg_class reltuple)
{
	if (reltuple->relkind == RELKIND_MATVIEW)
	{
		...
		if (ret == 3)
		{
			Oid			ruleId = get_rewrite_oid(reloid, NameStr(reltuple->relname), true);

			return OidIsValid(ruleId);
		}
	}
...
bool
vci_canreturn(Relation index, int attno)
{
	/*
...
{
	/*
	 * Datum		reloptions = PG_GETARG_DATUM(0); bool		validate =
	 * PG_GETARG_BOOL(1); bytea	   *result;
	 */


	/*
	 * ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("access
	 * method \"%s\" does not support storage_parameters", VCI_STRING)));
	 */
	bytea	   *result;

	result = default_reloptions(reloptions, validate, RELOPT_KIND_VCI);
...
bool
vci_gettuple(IndexScanDesc scan, ScanDirection dir)
{
	/*
...
int64
vci_getbitmap(IndexScanDesc scan, TIDBitmap *tbm)
{
	/*
	 * IndexScanDesc scan = (IndexScanDesc) PG_GETARG_POINTER(0); TIDBitmap
	 * *tbm = (TIDBitmap *) PG_GETARG_POINTER(1);
	 */
	int64		result;
...
vci_create_relation(const char *rel_identifier, Relation indexRel, IndexInfo *indexInfo, char vci_reltype)
{
	...
	switch (vci_reltype)
	{
		...
		case VCI_RELTYPE_WOS:
			...
			TupleDescInitEntry(new_tupdesc, (AttrNumber) 1, "orignal_tid", TIDOID, -1, 0);
...
CheckColumnReloptions(Relation indexRel, bool isctid)
{
	...
	{
		ids = ((char *) (indexRel)->rd_options) + ((StdRdOptions *) (indexRel)->rd_options)->vci_column_ids_offset;
		hasoption = !is_empty_cstring(ids);
	}
...
static bool
is_empty_cstring(const char *target)
{
	return (target == NULL || strlen(target) == 0);
}
...
get_tuple_desc_for_build(Relation heapRel, Relation indexRel, bool isctid)
{
	...
	Assert(isctid == !is_empty_cstring(ids));

	if (isctid)
		return vci_ExtractColumnDataUsingIds(ids, heapRel);
	else
...
{
	Bitmapset  *heapDroppedIds;
	Bitmapset  *vciDroppedIds;

	if (!isctid)
		return;

	/*
	 * We should distinguish if column ids in the table are changed when
	 * creating the VCI. While such change occurs in case of duplication or
	 * restoration, it does not in case of vci_create() or REINDEX (TRUNCATE).
	 *
	 * However, we cannot directly know such information. So compare dropped
	 * column ids (= A) in the table and vci_dropped_column_ids option (= B)
	 * in the VCI. - A is exist and A == B: vci_create() or REINDEX (TRUNCATE)
	 * (we should only return) - A is not exist and B is exist: duplication or
	 * restoration (we should fill dropped column ids) - Both A and B are not
	 * exist: Although we can't distinguish in this case, there is no problem
	 * because both returning and filling have same result. - Otherwise (A is
	 * exist and A != B): error
	 */
	heapDroppedIds = vci_create_dropped_column_ids(RelationGetDescr(heapRel));
	vciDroppedIds = vci_MakeDroppedColumnBitmap(indexRel);

	if (bms_is_empty(heapDroppedIds))
		vci_FillDroppedColumnIds(indexRel);
	else if (!bms_equal(heapDroppedIds, vciDroppedIds))
		ereport(ERROR,
				(errmsg("VCI failed to catch up with dropped columns in table"),
				 (errhint("Automatically recreated \"CREATE INDEX\" is corrupt. Please recreate VCI by \"vci_create()\" function."))));

	bms_free(heapDroppedIds);
	bms_free(vciDroppedIds);
}
...
vci_inner_build(Relation heapRel, Relation indexRel, IndexInfo *indexInfo)
{
	...
	/*
	 * When using 'vci_create', PostgreSQL registeres only a 'ctid' column as
	...
	vci_SetMainRelVar(vmr_info, vcimrv_xid_generation, 0, 1);	/* xid generation starts
																 * from 1 */
	vci_SetMainRelVar(vmr_info, vcimrv_xid_gen_udpate_xid, 0, GetCurrentTransactionId());
...
vci_add_drop_column(const ObjectAddress *object, int flags)
{
	...
	foreach(indexoidscan, indexoidlist)
	{
		...
		if (isVciIndexRelation(indexRel) &&
			vci_IsExtendedToMoreThan32Columns(indexRel))
		{
			Bitmapset  *indexedAttnums;
			int			attnum = object->objectSubId;

			indexedAttnums = vci_MakeIndexedColumnBitmap(indexOid,
														 CurrentMemoryContext,
														 NoLock);

			if (bms_is_member(attnum, indexedAttnums))
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				...
				vci_RegisterDroppedAttnum(indexRel, attnum);

			bms_free(indexedAttnums);
		}
...
vci_add_drop_relation(const ObjectAddress *object, int flags)
{
	...
	else
	{
		...
		/*
		 * Deletiion of VCI index by ALTER TABLE command is not supported
		 *
		 * Ereport only if the relation is vci main relation so that it does
		 * not give unneccesary messages.
		 *
...
static
Bitmapset *
vci_create_dropped_column_ids(TupleDesc tableTupdesc)
{
	int			i;
	Bitmapset  *attidsBitmap = NULL;

	for (i = 0; i < tableTupdesc->natts; i++)
	{
		Form_pg_attribute attr = TupleDescAttr(tableTupdesc, i);

		if (attr->attisdropped)
		{
			int			attid = attr->attnum - 1;

			Assert(!bms_is_member(attid, attidsBitmap));
			attidsBitmap = bms_add_member(attidsBitmap, attid);
		}
	}
