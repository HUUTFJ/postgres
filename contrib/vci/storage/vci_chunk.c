						  ...
						  vci_MainRelHeaderInfo *info)
{
	...
	rosChunkBuffer->nullData = palloc(rosChunkBuffer->nullWidthInByte *
									  numRowsAtOnce);
	rosChunkBuffer->tidData = useTid ? palloc(SizeOfIptrData * numRowsAtOnce)
		: NULL;
 ...
 * RosChunkBuffer.  The instance should be destroyed by
 * vci_DestroyRosChunkStorage().
 ...
 * @param[in] rosChunkStorage the target to be destroy.
 */
void
							   ...
							   TupleDesc tupleDesc)
{
	...
	if (rosChunkBuffer->tidData)
		MemCpy(&(rosChunkBuffer->tidData[SizeOfIptrData * offset]), tid, SizeOfIptrData);
				...
				case vcis_compression_type_fixed_raw:
					{
						...
						else
						{
							 ...
							 * Instead, (12U == size). Can we use beter way?
							 */
							Assert((12U == size) ||
					...
					/*
					 * We need the style to fill variable length data into
					 * fixed length area in order to reduce the space for the
					 * offsets and headers.
					 */
				case vcis_compression_type_variable_raw:
					{
						vci_offset_in_extent_t curOffset;
						vci_offset_in_extent_t size = VARSIZE_ANY(datum);
 ...
 * The created RosChunkBuffer should be destroy by
...
vci_CopyRosChunkBuffer(RosChunkBuffer *src)
{
	...
	RosChunkBuffer *dst = palloc(sizeof(RosChunkBuffer));

	CHECK_FOR_INTERRUPTS();

	vci_AllocateAndCopy(&dst, src, sizeof(RosChunkBuffer));

	vci_AllocateAndCopy(&(dst->compType),
						src->compType,
						sizeof(vcis_compression_type_t) * numColumns);
	vci_AllocateAndCopy(&(dst->nullBitId),
						src->nullBitId,
						sizeof(int16) * numColumns);
	vci_AllocateAndCopy(&(dst->columnSizeList),
						src->columnSizeList,
						sizeof(int16) * numColumns);
	vci_AllocateAndCopy(&(dst->nullData),
						src->nullData,
						src->nullWidthInByte * numFilled);
	vci_AllocateAndCopy(&(dst->tidData),
						src->tidData,
						SizeOfIptrData * numFilled);
	dst->deleteData = NULL;
	if (src->deleteData)
		vci_AllocateAndCopy(&(dst->deleteData),
							src->deleteData,
