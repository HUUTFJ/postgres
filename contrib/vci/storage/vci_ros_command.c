 ...
 *    In this case, all columns registerd to the VCI are converted into
 ...
 *    The precise description is,
 *    A. scan WOS with care of visibilty and deleted condition and collect
 *       visible TID.
 *    B. sort TID.
...
/**
 * @brief
 * This function is designed to detect transaction end after VCI exclusive
...
/**
 * @brief
 * This function detemine the result of the transaction status
 * of the previous ROS command.
 * First, it wait the end of the transaction of the previous if necessary.
 ...
 *
 * @note
 * Assuming that this function is called under main relaion is locked
 * exclusively.
 */
void
			...
			case VCI_XACT_DID_CRASH:
				{
					...
					elog(DEBUG1, "crush recovery: previous command=\"%s\"(%d)",
...
vci_CleanRosCommandContext(vci_RosCommandContext *comContext, bool neverWrite)
{
	...
	/* Close original heap relation if it is opend. */
...
/**
 * @brief Obtain number of rows in the relation estimated by ANALYLZE or
 * VACUUM commands.
...
/**
 * in vci_DestroyLocalRos(), release the memory context allocalted to the
...
vci_CollectDeletedRows(Relation mainRel, Size workareaSize, int32 extentId)
{
	...
	/* excute recovery previout ROS command if Necessary  */
	vci_RecoverOneVCIIfNecessary(&(comContext.info));
...
#if 0
		size = extentHead->size;
		type = extentHead->type;
#endif
...
truncateRos(vci_RosCommandContext *comContext)
{
	...
	for (colId = VCI_FIRST_NORMALCOLUMN_ID; colId < comContext->numColumns; ++colId)
	{
		...
		if (colInfo->comp_type != vcis_compression_type_fixed_raw)
		{
			...
			elog(DEBUG2, "  -- colId %d ,vaiable column ", colId);
