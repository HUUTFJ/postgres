 ...
 * vcimrv_xid_gen_udpate_xid stores 32-bit xid when the generation is lastly
 * advanced. E.g., either of XID which happened CREATE XID or VACUUM.
 *
 * While doing VACUUM, the upper 2 bit of vcimrv_xid_gen_udpate_xid and current
 ...
 * doing CREATE INDEX referres the index (can happen if the isolation level is
...
vci_GenerateXid64(TransactionId target_xid, vci_MainRelHeaderInfo *info)
{
	...
	base_xid = vci_GetMainRelVar(info, vcimrv_xid_gen_udpate_xid, 0);
...
vci_UpdateXidGeneration(vci_MainRelHeaderInfo *info)
{
	...
	if (diff != 0)
	{
		...
		vci_SetMainRelVar(info, vcimrv_xid_gen_udpate_xid, 0, cur_xid);
