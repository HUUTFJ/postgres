...
#define STABLE_CV_THRESHOLD (0.5)	/* a threshold of a coefficient of variation to judge the stability */
...
typedef struct
{
	...
	int			len;			/* the length of mountpoint. This value is stored avoiding multiple computations */
	char		devname[PATH_MAX];	/* device name */
	bool		mounted;		/* used for updating a mountpoint_dev_pair_t set. true means that this value remains */
	dlist_node	link;
