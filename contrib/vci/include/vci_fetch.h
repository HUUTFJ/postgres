...
typedef struct vci_CSFetchContextData
{
	...
	bool		use_column_store;	/* Store data in columnar style (true) or not. */
...
{
	bool		valid;			/* min and max are meaningful (true) or not (false). */
...
{
	vcirvs_read_whole,			/* Whole the data, that are required, are read. */
	...
	vcirvs_not_visible,			/* Failed to read since the extent is invisible. */
 ...
 * @param[in] localRos Local ROS to be destruct.
 */
static inline void
