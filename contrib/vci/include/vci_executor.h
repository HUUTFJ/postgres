...
typedef struct
{
	...
	Bitmapset  *attr_used;		/* Bitmap indicating the column position referenced in VCI index */
	int			num_fetches;	/* Number of VCI Scan that refer to VCI index of indexoid */

	vci_CSQueryContext query_context;	/* Column Store Query Context */
	vci_local_ros_t *volatile local_ros;	/* Pointer to Local ROS */

	vci_fetch_placeholder_t *fetch_ph_table;	/* Pointer to vci_fetch_placeholder_t array struct. Number of element in the array is num_fetches */
...
typedef struct
{
	...
	Instrumentation instrument; /* Instrumentation for aggregating Instrumentation of parallel workers during parallel execution */
} vci_plan_info_t;
...
typedef struct vci_query_context
{
	 ...
	 *
	 * Used to find lanState corresponding to plan in
...
typedef struct VciVPNode
{
	...
	vci_vp_item_id *arg_items;	/* Item number of the child VP node of this VP node */

	Datum	   *itemValue;		/* Datum array that records this VP node process result. Number of element is allocated VCI_MAX_FETCHING_ROWS. */
	bool	   *itemIsNull;		/* bool array that records this VP node process result. Number of element is allocated VCI_MAX_FETCHING_ROWS. */
...
typedef struct VciVPContext
{
	...
	vci_vp_item_id max_item;	/* Maximum number of nodes reserved by VP context */
	VciVPNode  *itemNode;		/* Array of VP node */

	Datum	   *resultValue;	/* Array of Datum that is the final result when VP context is processed */
	bool	   *resultIsNull;	/* Array of bool that is the final result when VP context is processed */

	vci_vp_item_id caseValue;	/* Temporarily records caseValue during execution of VciExecEvalVectorProcessing() */
	vci_vp_item_id domainValue; /* Temporarily records domainValue during execution of VciExecEvalVectorProcessing() */
...
typedef struct VciProjectionInfoSlot
{
	...
	union
	{
		...
		struct
		{
			int			expr_id;	/* Converted to pi_vp_tle_array[expr_id] in VciProjectionInfo */
		}			expr;
	}			data;
...
typedef struct VciProjectionInfo
{
	...
	ExprState	pi_state;
	TargetEntry **pi_tle_array; /* Array of expression state tree under TargetEntry that was converted */
	VciVPContext **pi_vp_tle_array; /* Array of VP context */
	int			pi_tle_array_len;	/* Maximum number of element of pi_vp_tle_array[] */
	ExprContext *pi_exprContext;	/* Execute context for executing this VciProjectionInfo */
	TupleTableSlot *pi_slot;	/* TupleTableSlot that contains this VciProjectionInfo result */
	bool		pi_directMap;
	int			pi_numSimpleVars;	/* Number of Simple Vars */
	int		   *pi_varSlotOffsets;	/* Pointer of mapping information used by Simple Vars */
	int		   *pi_varNumbers;	/* Pointer of mapping information used by Simple Vars */
	int		   *pi_varOutputCols;	/* Pointer of mapping information used by Simple Vars */
	VciProjectionInfoSlot *pi_slotMap;	/* Map information that records whether each target list was converted to SimpleVar or VP context. */
	int			pi_lastInnerVar;
	int			pi_lastOuterVar;
	int			pi_lastScanVar;
} VciProjectionInfo;
...
typedef struct VciPlan
{
	 ...
	 */
	int			preset_eflags;	/* eflags precalculated for parallel scheduling */

	AttrNumber	scan_plan_no;	/* Plan Number for VCI Scan that becomes a partitioned table */
...
typedef struct VciScan
{
	...
	bool		is_all_simple_vars; /* Target list is configured with SimpleVar */
	double		estimate_tuples;	/* Estimated number of rows in the scanned table	*/
	bool		is_subextent_grain; /* Execute sub-extent fine-grained parallelization or not */
	Index		index_ph_id;	/* index_ph_table[index_ph_id-1] of vci_query_context_t*/
	Index		fetch_ph_id;	/* index_ph_table[index_ph_id-1].fetch_ph_table[fetch_ph_id-1] of vci_query_context_t*/
} VciScan;
...
typedef struct VciScanState
{
	...
	vci_CSFetchContext local_fetch_context; /* Columnar fetch context (locale of each process) */
	...
	int		   *attr_map;		/* Map that substracts column store fetch id from Attr Number */

	int32		first_extent_id;	/* Extent number that starts reading */
	int32		last_extent_id; /* Extent number that finishes reading (exclusive) */
	int64		first_crid;		/* CRID that starts read */
	int64		last_crid;		/* CRID taht finishes read(exclusive) */
	...
	VciFetchPos mark;			/* Column store fetch location recorded in mergr */
	...
	/***
	 * true when parallel worker recive NULL
	 */
	bool		scan_done;
...
typedef struct VciAggState
{
	...
	VciProjectionInfo *vps_ProjInfo;	/* ProjectionInfo when generating Agg State output */
	...
	Datum	  **hash_input_values;	/* array of pointers to datum vector for each hash key */
	bool	  **hash_input_isnull;	/* array of pointers to null vector for each ehash key */
...
typedef struct VciVarState
{
	...
	VciScanState *scanstate;	/* Pointer to VciScanState from which Var will load data */
} VciVarState;
...
extern void vci_finish_vecotr_set_from_column_store(VciScanState *scanstate);
