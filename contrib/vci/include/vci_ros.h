 ...
 * calculated from the position.  The position and offsetInPage
 * is measured in data area in DB pages.  We do not care the
 * header of DB page in this macro.
...
typedef enum vci_MainRelVar
{
	...
	vcimrv_xid_gen_udpate_xid = 0x00030050,
...
typedef struct vci_MainRelHeaderInfo
{
	 ...
	 */
	Buffer		buffer[VCI_NUM_MAIN_REL_HEADER_PAGES];	/* Buffers for the main relation header pages. */
...
/**
 * @brief
 * VCI main relation header area to store by vci_WriteMainRelVar()
 ...
 * given in CREATE INDEX command.
 * The function below gives how many colums or data belong
...
typedef struct
{
	...
	Oid			dbid;			/* Oid of database to which a VCI main relation belongs */
	bool		force_next_wosros_conv; /* flag to force WOS->ROS conversion on next time */
