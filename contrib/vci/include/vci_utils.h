...
/**
 * @brief Allocate memory area with given size, and copy the give source
 * to the area newly allocated.
 * @param[out] dst Memory area with size byte is allocated, and the pointer
 * is stored into *dst.
 * So give it a pointer of pointer.
 * The data in *src are copied in **dst.
 * @param[in] src Pointer to the array of byte data to be copied.
 * @param[in] size The size of source data pointed by src.
 */
static inline void
vci_AllocateAndCopy(void *dst, void *src, Size size)
{
	if (src && (0 < size))
	{
		dst = palloc(size);
