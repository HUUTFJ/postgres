...
typedef struct VciShmemStruct
{
	...
	int			num_devload_info;	/* monitored device numbers + 1(for unmonitored devices) */
	int			max_devices;	/* max device num for storage */
	int			translated_dev_pos; /* index of a device VCIs on which is to be translated */
	LWLock	   *io_load_lock;
