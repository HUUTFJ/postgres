...
/**
 * @brief Context for ROS commands,
 * containing TID list read from data WOS or whiteout WOS,
 * data read from the PostgreSQL heap relation or from ROS,
...
typedef struct vci_RosCommandContext
{
	...
	int32		extentId;
	int32		extentIdSrc;	/* source extentId in copy operation (wos2ros, cdr) */
...
extern uint64 vci_ConvertWos2RosForBuild(Relation mainRel, Size workarea, IndexInfo *indexInfo);
