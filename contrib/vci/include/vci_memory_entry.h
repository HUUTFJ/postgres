...
typedef struct
{
	...
	Oid			dbid;			/* Oid of database where VCI main relations belongs */
} vci_id_t;
...
typedef struct
{
	...
	Oid			tsid;			/* Oid of tablespace where VCI a main relation belongs */

	/**
	 * If tsid is equal to InvalidOid, the Oid correspoing to default table space.
	 * Otherwise, this is equal to tsid.
	 */
	Oid			real_tsid;
 ...
 * be allocated in shared memory living throught the PostgreSQL instance.
 */
typedef struct
{
	/**
