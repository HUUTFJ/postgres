...
/**
 * Attempt to rewrite plan and return the rewritten planned stmt is sucessful.
 *
...
vci_generate_custom_plan(PlannedStmt *src, int eflags, Snapshot snapshot)
{
	...
	if (!changed)
	{
		elog(DEBUG1, "No plan to be rewrttien");
		return NULL;
	}
	 ...
	 * Material nodoe if eflag needs them.
	 */
...
vci_contain_nestloop_param_expr_walker(Node *node, void *context)
{
	...
	if (nodeTag(node) == T_Param)
	{
		...
		{
			elog(DEBUG1, "Param contains non-permmitted paramId");
			return true;
		}
	}
...
vci_satisfies_vci_join(vci_rewrite_plan_context_t *rp_context, Join *join)
{
	 ...
	 * stricker restrictions are imposed than on outer.
	 */
...
/**
 * Determine whether t egiven oid is not user defined type
 */
static bool
