/*-------------------------------------------------------------------------
 *
 * vci_planner_prepare.c
 *	  Preprocessing for plan rewrite routine
 *
 * Portions Copyright (c) 2025, PostgreSQL Global Development Group
 *
 * IDENTIFICATION
 *		contrib/vci/executor/vci_planner_prepare.c
 */
#include "postgres.h"
 ...
 * @param[in]     parent   Pointer to  plane node that is the parent of *plan_p
 * @param[in,out] context  Pointer to plan rewrite information
 * @param[in]     eflags   execution plan
...
preanalyze_plan_tree_mutator(Plan **plan_p, Plan *parent, void *context, int eflags, bool *changed)
{
	...
	switch (nodeTag(plan))
	{
		case T_SubqueryScan:	/* Since using VCI cusom scan for initplans
								 * slows down the performance, block VCI scan
								 * to be replaced for subqueryscan */
			return true;
		case T_ModifyTable:
		case T_TidScan:
		case T_TidRangeScan:
	...
	rp_context->plan_attr_map[plan_no].plan_compat = rp_context->forbid_parallel_exec ? VCI_PLAN_COMPAT_FROBID_TYPE : VCI_PLAN_COMPAT_OK;
