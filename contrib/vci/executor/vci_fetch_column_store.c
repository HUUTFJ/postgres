 ...
 *	  Routine to fecth from column store
 *
...
create_all_queries_context_for_fetching_column_store(QueryDesc *queryDesc, int eflags)
{
	 ...
	 *
	 * Multiple queries simultanously creating Local ROS arw counted by
	 * num_standby_exec_queires. Restart WAL replay at the end of last query.
	 */
...
enter_standby_query(void)
{
	...
	if (VciShmemAddr->num_standby_exec_queires == 0)
		SetVciRecoveryPause();
	VciShmemAddr->num_standby_exec_queires++;
 ...
 */
void
vci_finish_vecotr_set_from_column_store(VciScanState *scanstate)
{
