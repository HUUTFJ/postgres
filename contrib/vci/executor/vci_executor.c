 ...
 *
 * @note There are pattern in which Executor is recursively called, such as when stored procedure is called
...
vci_subxact_change_handler(SubXactEvent event, SubTransactionId mySubid)
{
	switch (event)
	{
		...
		case SUBXACT_EVENT_ABORT_SUB:
			...
			{
				elog(DEBUG1, "vci:executor caught any exception in sub transavtion");
				vci_free_query_context();
